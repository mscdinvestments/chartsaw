...
const G = `
  ...
  :root {
    --gold:#C9A84C; --gold-light:#E8C96A; --green:#2ECC71; --red:#FF6B6B;
    --yellow:#F4D03F; --bg:#0C0F14; --bg2:#131820; --card:#161D26;
    --border:#243040; --border2:#2E3D50; --text:#E8EEF4; --text2:#A8B8C8;
    --muted:#607080; --radius:6px;
  }
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .sn{width:24px;height:24px;background:rgba(201,168,76,0.1);border:1px solid rgba(201,168,76,0.3);color:var(--gold);font-family:'Bebas Neue',sans-serif;font-size:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;border-radius:4px;}
  ...
  .t-sym{font-family:'Bebas Neue',sans-serif;font-size:64px;color:var(--gold);letter-spacing:5px;line-height:1;text-shadow:0 0 30px rgba(201,168,76,0.2);}
  ...
  .sc_{background:var(--card);border:1px solid var(--border);padding:14px 16px;border-radius:var(--radius);}
  .sl_{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin-bottom:6px;}
  .sv_{font-family:'IBM Plex Mono',monospace;font-size:18px;font-weight:600;}
  .sn_{font-size:11px;color:var(--text2);margin-top:4px;font-style:italic;}
  ...
  .tbl td{padding:12px;border-bottom:1px solid rgba(36,48,64,0.6);font-size:13px;vertical-align:top;line-height:1.6;}
  .tbl tr:last-child td{border-bottom:none;}
  .tbl tr:hover td{background:rgba(255,255,255,0.025);}
  .tbl-tool{font-size:13px;font-weight:600;color:var(--text);}
  .tbl-sub{font-size:10px;color:var(--muted);margin-top:3px;letter-spacing:.5px;}
  .tbl-read{color:var(--text);}
  .tbl-imp{color:var(--text2);font-size:12px;}
  ...
  .lr_{display:flex;align-items:center;padding:10px 14px;border-bottom:1px solid rgba(36,48,64,0.4);}
  .lr_:last-child{border-bottom:none;}
  ...
  .sr_{display:flex;justify-content:space-between;align-items:baseline;padding:6px 0;font-size:12px;border-bottom:1px solid rgba(36,48,64,0.35);gap:8px;}
  .sr_:last-child{border-bottom:none;}
  .sk_{color:var(--text2);flex-shrink:0;}
  .sv2_{font-family:'IBM Plex Mono',monospace;font-weight:600;text-align:right;color:var(--text);word-break:break-word;}
  ...
  @media(max-width:768px){
    ...
    .inp-prices,.inp-actions{grid-template-columns:1fr;gap:8px;}
    ...
    .tbl td{display:block;padding:6px 0;border-bottom:none;font-size:13px;}
    .tbl td:first-child{border-bottom:1px solid var(--border);padding-bottom:10px;margin-bottom:8px;}
    .tbl td:nth-child(3)::before{content:'Reading: ';font-weight:600;color:var(--muted);font-size:10px;letter-spacing:1px;text-transform:uppercase;display:block;margin-bottom:4px;}
    .tbl td:nth-child(4)::before{content:'Implication: ';font-weight:600;color:var(--muted);font-size:10px;letter-spacing:1px;text-transform:uppercase;display:block;margin-bottom:4px;margin-top:8px;}
      ...
      <div>
        <header className="hdr">
          <div className="hdr-logo">
            ...
            <div>
              ...
              <div className="hdr-sub">Cutting through noise · <span>Soloway PPT Methodology</span></div>
            </div>
          </div>
          <div className="hdr-right">
            <div className="hdr-badge">Price · Pattern · Time</div>
        ...
        <div className="inp">
          ...
          <div className="inp-main">
            <F label="Ticker Symbol *">
              <input value={p.ticker}
                onChange={e => { up('ticker', e.target.value.toUpperCase()); setLive(null); setFilled({}) }}
                placeholder="e.g. HYMC" maxLength={10}
                onKeyDown={e => e.key === 'Enter' && loadLive()} />
            </F>
            <F label="Current Price · 52-Week High · 52-Week Low">
              <div className="inp-prices">
                {[['currentPrice','Current Price'],['weekHigh52','52W High'],['weekLow52','52W Low']].map(([k,ph]) => (
                  <input key={k} className={filled[k]?'live':''} value={p[k]}
                    onChange={e => up(k, e.target.value)} placeholder={ph} />
                ))}
              </div>
            </F>
          </div>
          ...
          {live && (
            <div className="lstrip">
              {[
                ['Company',   <span className="lname">{live.companyName}</span>],
                ['Price',     <span className="lsv ov">${live.currentPrice}</span>],
                ['Change',    <span className={`lsv ${parseFloat(live.change)>=0?'gv':'rv'}`}>{parseFloat(live.change)>=0?'+':''}{live.change} ({parseFloat(live.changePct)>=0?'+':''}{live.changePct}%)</span>],
                ['52W High',  <span className="lsv ov">${live.weekHigh52}</span>],
                ['52W Low',   <span className="lsv ov">${live.weekLow52}</span>],
                live.beta        && ['Beta',       <span className="lsv">{live.beta}</span>],
                live.peRatio     && ['P/E',        <span className="lsv">{live.peRatio}</span>],
                live.weekReturn52&& ['52W Return', <span className={`lsv ${parseFloat(live.weekReturn52)>=0?'gv':'rv'}`}>{live.weekReturn52}%</span>],
              ].filter(Boolean).map(([label,val],i) => (
                <div key={i} className="lsi">
                  <div className="lsl">{label}</div>{val}
                </div>
              ))}
          ...
          <button className="advbtn" onClick={() => setAdv(x => !x)}>
            {adv?'▲':'▼'} {adv?'HIDE':'SHOW'} ADVANCED PARAMETERS
          </button>
              ...
              {[
                ...
                <F key={key} label={label}>
                  {type==='sel'
                    ...
                    : <input className={filled[key]?'live':''} value={p[key]} onChange={e=>up(key,e.target.value)} placeholder={opts}/>}
                </F>
              ))}
              <F label="Catalysts / Context Notes" style={{gridColumn:'span 3'}}>
                <input value={p.catalysts} onChange={e=>up('catalysts',e.target.value)}
                  placeholder="e.g. GDXJ inclusion, earnings beat, Fed meeting, PEA due Q2, insider buying..."/>
              </F>
              <F label="Timeframe">
        ...
        {!running && !a && (
          <div className="empty">
            ...
            <div className="empty-sub">Enter any ticker, load live market data, then run a full Gareth Soloway PPT analysis — Fibonacci levels, support/resistance, patterns, trade setups, and more.</div>
            <div className="steps">
              {[['1','Enter Ticker'],['2','Load Live Data'],['3','Run Analysis']].map(([n,t])=>(
                <div key={n} className="step"><div className="sn">{n}</div>{t}</div>
              ))}
            </div>
          </div>
        )}
        ...
        {!running && a && (
          <div className="res">
            <div className="tkr-row">
              <div>
                <div>
                  ...
                  {a.changePct!=='0' && (
                    <span className="t-chg" style={{color:parseFloat(a.changePct)>=0?'var(--green)':'var(--red)'}}>
              ...
              <div className="t-date">CHARTSAW · {new Date().toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</div>
            </div>
            ...
            {a.stats.length>0&&<>
              ...
              <div className="sg">
                {a.stats.map((s,i)=>(
                  <div key={i} className="sc_">
                    <div className="sl_">{s.label}</div>
                    <div className="sv_" style={{color:sc(s.color)}}>{s.value}</div>
                    {s.note&&<div className="sn_">{s.note}</div>}
                  </div>
                ))}
              </div>
            </>}
            ...
            <table className="tbl">
              <thead><tr>{[['Soloway Tool','18%'],['Signal','12%'],['Reading','38%'],['Implication','32%']].map(([h,w])=><th key={h} style={{width:w}}>{h}</th>)}</tr></thead>
              <tbody>
                {a.signals.map((s,i)=>(
                  <tr key={i}>
                    <td><div className="tbl-tool">{s.tool}</div>{s.sub&&<div className="tbl-sub">{s.sub}</div>}</td>
                    <td><span className={`badge ${bc(s.sig)}`}>{s.sig}</span></td>
                    <td className="tbl-read">{s.read}</td>
            ...
            <div className="lvlg">
              {[['▼ DOWNSIDE / SUPPORT TARGETS',a.down,'lhr','var(--red)'],['▲ UPSIDE / RESISTANCE TARGETS',a.up2,'lhg','var(--green)']].map(([title,levels,hcls,pc])=>(
                <div key={title} className="lb">
                  <div className={`lbh ${hcls}`}>{title}</div>
                  {levels.map((l,i)=>(
                    <div key={i} className="lr_">
                      <div className="lrt">{l.type}</div>
                      <div className="lrp" style={{color:pc}}>{l.price}</div>
            ...
            <div className="tbg">
              {[['BULL THESIS',a.bull,'var(--gold)','thc','tct'],['BEAR RISKS',a.bear,'var(--red)','rkc','rct']].map(([title,items,color,cls,tcls])=>(
                <div key={title}>
                  <div className="sec">{title}</div>
                  {items.map((item,i)=>(
                    <div key={i} className={cls}>
                      <div className={tcls}>{item.title}</div>
            ...
            <div className="stg">
              {[['▲ BULL SETUP — Swing Long',a.bullSetup,'shg'],['▼ BEAR SETUP — Short / Avoid',a.bearSetup,'shr']].map(([title,setup,cls])=>(
                <div key={title} className="stc">
                  <div className={`sth ${cls}`}>{title}</div>
                  {Object.entries(setup).map(([k,v])=>(
                    <div key={k} className="sr_">
                      <span className="sk_">{k}</span>
                      <span className="sv2_">{v}</span>
                    </div>
                  ))}
                </div>
              ))}
            ...
            <div className="footer">
              ...
              <div className="footer-disc">CHARTSAW applies Gareth Soloway's publicly documented PPT Methodology for educational and informational purposes only. Not financial advice. Always conduct your own due diligence. Trading involves substantial risk of loss.</div>
            </div>
          </div>
        )}
      </div>
